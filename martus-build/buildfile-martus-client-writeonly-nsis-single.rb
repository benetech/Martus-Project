name = 'martus-client-writeonly-nsis-single'
...
define name, :layout=>create_layout_with_source_as_source('.') do
	...
	exe_name = "MartusWriteOnlySetupSingle.exe"
	exe_path = _(:target, exe_name)
	destination = _(:target, "MartusClientSetup-Writeonly-#{project.version}-#{input_build_number}-#{release_build_number}.exe")
	...
	file exe_path => nsis_zip_file do
		...
		run_nsis_task(nsis_zip_file.to_s, 'NSIS_Martus_Writeonly_Single.nsi', exe_name)
