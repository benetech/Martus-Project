name = 'martus-client-writeonly-nsis-upgrade'
...
define name, :layout=>create_layout_with_source_as_source('.') do
  ...
  file exe_path => nsis_zip_file do
    puts "Building NSIS upgrade installer"
		run_nsis_task(nsis_zip_file.to_s, 'NSIS_Martus_Writeonly_Upgrade.nsi', exe_name)
    destination = _(:target, "MartusClientWriteonlyUpgrade-#{project.version}-#{input_build_number}-#{release_build_number}.exe")
