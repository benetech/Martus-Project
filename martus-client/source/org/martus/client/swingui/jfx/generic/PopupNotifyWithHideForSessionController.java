...
public class PopupNotifyWithHideForSessionController extends PopupNotifyController
{
	public PopupNotifyWithHideForSessionController(	UiMainWindow mainWindowToUse, String notificationTag)
	{
		super(mainWindowToUse, notificationTag);
		
		if(hiddenStatuses ==  null)
			hiddenStatuses = new HashMap<String, SimpleBooleanProperty>();
		
		shouldBeHidden = hiddenStatuses.get(notificationTag);
		if(shouldBeHidden == null)
		{
			shouldBeHidden = new SimpleBooleanProperty();
			hiddenStatuses.put(notificationTag, shouldBeHidden);
		}
	}
